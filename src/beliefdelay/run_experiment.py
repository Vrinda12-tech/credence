...
def tune_lr(arch, env, cfg, P, grid, device, frac=0.5):
    """Held-out-CE (oracle-free) LR search at `frac` of the training length; extends past grid edges, defaults on a flat grid."""
    def score(lr):
        c = dict(cfg, lr=lr, steps=max(50, int(cfg["steps"] * frac)))
        torch.manual_seed(999)
        m = build_matched(arch, env.token_fields, env.n_out, P, max_len=cfg["L"]).to(device)
    ...
    return adaptive_lr_search(score, grid, default=cfg["lr"])
                ...
                for seed in seeds:
                    ...
                    if not args.no_tune:
                        ...
                        if os.path.exists(lrp):
                            ...
                            lr_info = tune_lr(arch, env, cfg, P, grid, device)
                    ...
                    res.update(converged=not convergence_flag(hist, run_cfg["steps"]), lr_at_bound=(lr_info["at_bound"] if not args.no_tune else None),
                               lr_flat=(lr_info["flat"] if not args.no_tune else None), env=env_name, arch=arch, seed=seed, target_params=P, n_params=model.n_params(),
